...
pop <- read_delim("0. Docs/PopulacaoEvolucaoMensalCisp.csv", 
...
isp_novo_0 <- left_join(isp_novo, nomes_dp)
...
crimes_pop_ano <- left_join(crimes, pop_ano) %>%
  mutate(perct_ltd_violenta = (sum_ltd_violenta/sum_pop)*100000,
         ...
         ind_ltd_violenta = sum_autos_resis/sum_pop,
  ) %>%
  mutate(ind_ltd_violenta_pop = (ind_ltd_violenta/sum_ltd_violenta)*100000)
